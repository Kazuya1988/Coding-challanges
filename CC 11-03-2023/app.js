...
function wordsToHex(words) {
  ...
  return arrOfWords
    ...
    .map((c) =>
      c.length < 3
        ? c.length < 2
          ? `#${charToASCIItoHex(c[0])}0000`
          : `#${charToASCIItoHex(c[0])}${charToASCIItoHex(c[1])}00`
        : `#${charToASCIItoHex(c[0])}${charToASCIItoHex(
            c[1]
          )}${charToASCIItoHex(c[2])}`
    );
  // Clever
...
function charToASCIItoHex(character) {
  let asciiChar = character.charCodeAt(0);
  return asciiChar.toString(16);
}
