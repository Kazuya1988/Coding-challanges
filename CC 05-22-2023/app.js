...
function iterPi(epsilon) {
	...
	while (Math.abs(leibnizSeries * 4 - Math.PI) >= epsilon) {
		leibnizSeries += (i % 2 === 0 ? 1 : -1) / (2 * i + 1);
		i++;
	}

	let approximation = (leibnizSeries * 4).toFixed(10);
	return [i, parseFloat(approximation)];
}
