...
function isLanguageDiverse(list) {
	let countLanguages = list.reduce((obj, user) => {
		obj[user.language] = (obj[user.language] || 0) + 1;
		return obj;
	}, {});
	console.log(countLanguages);
	let numberOfLanguages = Object.values(countLanguages);
	return Math.max(...numberOfLanguages) <= Math.min(...numberOfLanguages) * 2;
}
