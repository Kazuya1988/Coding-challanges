...
// 		peaks: [],
// 	};
// 	if (arr.lastIndexOf(Math.max(...arr)) >= arr.length - 1) {
// 		return localMaxima;
// 	}
// 	for (let i = 0; i < arr.length; i++) {
// 		if (arr[i] >= arr[i + 1] && arr[i] > arr[i - 1]) {
// 			localMaxima.pos.push(i);
// 			localMaxima.peaks.push(arr[i]);
// 		}
// 	}
// 	return localMaxima;
// }
function pickPeaks(arr) {
	const localMaxima = { pos: [], peaks: [] };
	...
	for (let i = 1; i < arr.length - 1; i++) {
		if (arr[i] > arr[i - 1]) {
			...
			if (arr[i] > arr[i + 1]) {
				localMaxima.pos.push(plateauStart);
				localMaxima.peaks.push(arr[plateauStart]);
			}
		}
	}
