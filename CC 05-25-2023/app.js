...
function autocomplete(input, dictionary) {
	let cleanedInput = input.replace(/[^a-z]/gi, '');
	let dictionaryFiltered = dictionary.filter(
		(e) =>
			e.slice(0, cleanedInput.length) === cleanedInput ||
			e.slice(0, cleanedInput.length).toLowerCase() === cleanedInput
	);
	return dictionaryFiltered.slice(0, 5);
}
console.log(autocomplete('ai', ['airplane', 'airport', 'apple', 'ball']));
console.log(autocomplete('a', ['airplane', 'airport', 'Apple', 'ball']));
console.log(autocomplete('a', ['irplane', 'irport', 'pple', 'ball']));
